...
public class PictureRepository : IPictureRepository
{
    ...
    {
        var currentMainPicture = await _context.Pictures.Include(x => x.Posts).Where(x => x.Posts.Select(x => x.Id).Contains(postId)).SingleOrDefaultAsync(x => x.Main);
        currentMainPicture.Main = false;

        var newMainPicture = await _context.Pictures.SingleOrDefaultAsync(x => x.Id == id);
